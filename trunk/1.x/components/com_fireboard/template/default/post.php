<?php
/**
* @version $Id: post.php 511 2007-12-25 danialt $
* Fireboard Component
    ...
    <tr>
                    ...
                    <tr>
                        <td>
                            ...
                            else
                            {
                                ...
                                {
                                    $database->setQuery(
                                    "INSERT INTO #__fb_messages (parent,thread,catid,name,userid,email,subject,time,ip,topic_emoticon,hold,poll_id) VALUES('$parent','$thread','$catid','$fb_authorname','{$my->id}','$email','$subject','$posttime','$ip','$topic_emoticon','$holdPost','$poll')"); // POLL
    			                    ...
    			                    if ($database->query()) 
                                    {
                                        ...
                                        // POLL
				    					$database->setQuery("UPDATE #__polls SET published=1 WHERE id='$poll'");
                                    	$database->query();
                                    	
                                    	if(!session_id())  /* Destroying the Poll Security Session */
                                    	{
                                    		session_start();
                                    		if(isset($_SESSION['kEy']))
                                    			{ $_SESSION['kEy']=""; session_unset(); }
                                    	}
		...
		<?php
            ...
            ?>
                ...
                /* Poll System: Add vote */
				else if ($do == "addvote" && isset($_POST['vsubmit'])  )
                {
					$p_id=$_POST["pID"];
					$p_type=$_POST["pType"];
					$u_id=$_POST["userID"];
					echo "<center>Thank you for your vote.<br>&nbsp;</center>";
			
					if($p_type==1) // single choice
					{
						$v_id=$_POST["myoption"];
						if($v_id>0)
						{
							$database->setQuery("INSERT INTO #__fb_poll_result (uid, v_option) VALUES ('{$u_id}', '{$v_id}')");
							$database->query();
						}
					}
					else // multiple choice
					{
						$database->setQuery("BEGIN"); $database->query();
						for($i=0;$i<12;$i++)
						{
							$v_id=$_POST["myoptions".($i+1)];
							if($v_id>0)
							{
								$database->setQuery("INSERT INTO #__fb_poll_result (uid, v_option) VALUES ('{$u_id}', '{$v_id}')");
								$database->query();
						...
						$database->setQuery("COMMIT"); $database->query();
					}
					?>
					<script language = "javascript">
...
{
    $database->setQuery('SELECT id,catid,parent,thread,subject,userid,poll_id FROM #__fb_messages WHERE id=' . $id); // POLL
    ...
    {
	    // Delete Forum votes
	    $database->setQuery("DELETE FROM #__fb_poll_result WHERE v_option IN ( SELECT id FROM #__poll_data WHERE pollid='{$mes->poll_id}' )" );
	    $database->query();
	    
	    // Unpublish the Poll
	    $database->setQuery("UPDATE #__polls SET published=0 WHERE id='{$mes->poll_id}'");
	    $database->query();
    }
