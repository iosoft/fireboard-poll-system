...
function savePoll( $option ) {
	...
	foreach ($options as $i=>$text) {
		...
		if(trim($text)!="") /* FB-Poll : removes NULL options from the Database */
		if ($isNew) {
...
function removePoll( $cid, $option ) {
	...
	{
		/* FB-Poll: Delete votes */
		$database->setQuery("DELETE FROM #__fb_poll_result WHERE v_option IN ( SELECT id FROM #__poll_data WHERE pollid=".$cid[$i].")");
		$database->query();
		///////////////////////////
		$poll = new mosPoll( $database );
		if (!$poll->delete( $cid[$i] ))
		{
			$database->setQuery("ROLLBACK"); $database->query();
			$msg .= $poll->getError();
			break;
		}
	}
